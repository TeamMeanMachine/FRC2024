...
suspend fun pickUpSeenNote() = use(Drive, Intake, name = "pick up seen note") {
    ...
    if (NoteDetector.seesNotes) {
        ...
        periodic {
            ...
            if (!useEstimation) {
                ...
                val fieldPosition = notePos.rotateDegrees(Drive.heading.asDegrees) + Drive.combinedPosition
            ...
            if (useEstimation && notePosCount > 0) {
                ...
                headingError = (noteFieldPose - Drive.combinedPosition).angleAsDegrees - Drive.heading.asDegrees
                notePos = (noteEstimatedPosition/notePosCount.toDouble() - Drive.combinedPosition).rotateDegrees(-Drive.heading.asDegrees)
            }
            ...
            if (notePos != Vector2(0.0, 0.0)) {
                ...
                println("Use estimation: $useEstimation")
                println("Heading Error $headingError")
                println("Note position $notePos")
                ...
                Drive.drive(notePos.normalize(), turnControl * 0.4, false)
            ...
            if (OI.driveTranslation.x > 0.1 || OI.driveTranslation.y > 0.0 || OI.driveRotation > 0.1) {
                println("stopping because of input")
                stop()
            }


        }
        print("done picking up note")
    }
}
